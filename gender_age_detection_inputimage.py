...
def detect(gray, frame): # Function that takes as input the image in black and white (gray) and the original image (frame), and that will return the same image with the detector rectangles. 
    ...
    for (x, y, w, h) in faces: # For each detected face:
        ...
        cv2.putText(frame, overlay_text_g, (x, y), font, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(frame, overlay_text_a, (x, y+h), font, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
...
while True: 
    ...
    #frame= cv2.imread(r'C:\Users\LAPTOP\Desktop\images_female_p\p\i271ta-fg.jpg') # We get the last frame.
    frame= cv2.imread(r'F:\Images\Capture.jpg')
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) 
    canvas = detect(gray, frame) # Output of our detect function.
    cv2.imshow('Input Image', canvas) 
